"""
Tokenize four_pillars_mastery.jsonl using tokenizer_v2.model and append into train.bin.
"""
...
JSONL_PATH = Path("data/raw/corpus_v2/four_pillars_mastery.jsonl")
OUT_BIN = Path("data/processed/corpus_v2/four_pillars_mastery.bin")
    ...
    with open(JSONL_PATH, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if doc_count % 10000 == 0:
                print(f"  {doc_count:,} passages tokenized | {total_tokens:,} tokens", flush=True)

    dt = time.time() - t0
    print(f"\n[DONE] Tokenized {doc_count:,} passages -> {total_tokens:,} tokens in {dt:.1f}s ({total_tokens / dt:,.0f} tok/s)")
