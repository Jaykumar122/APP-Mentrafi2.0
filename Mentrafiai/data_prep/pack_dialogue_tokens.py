"""
Tokenize conversational_dialogues.jsonl using tokenizer_v2.model and append into train.bin.
"""
...
JSONL_PATH = Path("data/raw/corpus_v2/conversational_dialogues.jsonl")
OUT_BIN = Path("data/processed/corpus_v2/conversational_dialogues.bin")
    ...
    with open(JSONL_PATH, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if doc_count % 10000 == 0:
                print(f"  {doc_count:,} dialogues tokenized | {total_tokens:,} tokens", flush=True)

    dt = time.time() - t0
    print(f"\n[DONE] Tokenized {doc_count:,} dialogues -> {total_tokens:,} tokens in {dt:.1f}s ({total_tokens / dt:,.0f} tok/s)")
