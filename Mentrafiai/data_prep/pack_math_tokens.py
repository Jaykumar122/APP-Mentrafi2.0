"""
Tokenize advanced_finance_math.jsonl using tokenizer_v2.model and append into train.bin.
"""
...
JSONL_PATH = Path("data/raw/corpus_v2/advanced_finance_math.jsonl")
OUT_BIN = Path("data/processed/corpus_v2/advanced_finance_math.bin")
    ...
    with open(JSONL_PATH, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if doc_count % 10000 == 0:
                print(f"  {doc_count:,} docs tokenized | {total_tokens:,} tokens", flush=True)

    dt = time.time() - t0
    print(f"\n[DONE] Tokenized {doc_count:,} docs -> {total_tokens:,} tokens in {dt:.1f}s ({total_tokens / dt:,.0f} tok/s)")
